...
- **Task 2/3**:JWKS **未**做进程内缓存。轮换由独立 CLI 进程执行,进程内缓存会让运行中的服务持续发布过期密钥集,因此 `publicJwks()` 每次实时构建。同时 `kid` 需通过 `/^[A-Za-z0-9_-]{1,64}$/` 校验,以阻断 `../` 之类的路径穿越。
- **Task 6**:计划中的 `test/token-ttl.test.ts` 未落地,改为在 `test/run-smoke.mjs` 内断言真实 TTL(默认 600s、环境变量覆盖、响应体 `expires_in` 一致)。原计划的单测只验证了 jose 的签名行为,并未覆盖本仓库代码。
- **Task 9**:`client_secret` 为空串或非字符串时在加载阶段直接拒绝(此前会退化成 `safeEqual('','')` 通过,构成鉴权绕过);`${ENV:...}` 占位格式非法时抛错,而不是被当成字面量 secret 静默生效;引用的环境变量缺失或为空串同样抛错。
- **Task 15**:admin 会话 TTL 收敛到 `admin/src/index.ts` 中唯一且经过校验的配置源。若沿用裸数值 `ADMIN_SESSION_TTL`,fast-jwt 会把数字按毫秒解析,得到秒级有效期;空值则产生永不过期的 token。
- `clients.json` 已移出版本控制,泄露的 secret 已完成轮换;git 历史**有意不重写**。
